...
def load_data(database_filepath):
    ...
    Args:
    dadatabase_filepath: Path and name of the database that will be openned 
    
    Returns:
    X: dataframe. variables that will help us in the prediction process
    Y: dataframe. columns that we will predict
    """     
...
def tokenize(text):
    """ Tokenizen process of words
    
    Args:
    text: string. text 
    
    Returns:
    List containing normalized word tokens
    """
...
def build_model():
    """ Build all Pipeline model
    
    Args:
 
    
    Returns:
    grid search model with the best parameters
    """
...
def evaluate_model(model, X_test, Y_test, category_names):
    """Returns test accuracy, precision, recall and F1 score for our model
...
def save_model(model, model_filepath):
    """Save our model in a pickle file
    
    Args:
    model: object. Fitted model object.
    model_filepath: string. path and name for our pickle archive.
